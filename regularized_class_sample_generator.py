...
class RegularizedClassSpecificImageGeneration():
    ...
    def __init__(self, model, target_class,iterations,class_name):
        ...
        if not os.path.exists(f'generated_class/class_{self.class_name}'):
            os.makedirs(f'generated_class/class_{self.class_name}')
    ...
    def generate(self, blur_freq=5, blur_rad=1, wd=0.0001, clipping_value=0.1):
        ...
        with open(f'generated_class/class_{self.class_name}/run_details.txt', 'w') as f:
        ...
        os.rename(f'generated_class/class_{self.class_name}',
...
def run_from_import(target_class, network, iterations,class_name):
    pretrained_model = getattr(models,network)(pretrained=True)
    print(network + " selected generating image...")
    csig = RegularizedClassSpecificImageGeneration(pretrained_model, target_class, iterations,class_name)
    csig.generate()

if __name__ == '__main__':
    target_class = 115  # Snail
